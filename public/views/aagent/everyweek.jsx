		...
		http.get('/dudai/?c=analysis.report&ac=get&token=mgame_afs23cgs23', {params: data})
			...
			.then((data)=> {
				if (data.error_code === 0) {
					...
					Chart.handleShowChart('c2', response, ['付费' + dimensionText, '充值收入',], ['日期', dimensionText + '周ARPPU', dimensionText + '周付费率']);
	...
	handleAccountData: function (data, dimensionText) {
		...
		let mean = {
			...
			mean_cash: _.meanBy(data, (m)=> {
				return parseFloat(m[dimensionText + '周付费率'])
			})
		};
